...
class BatchSamplerSafe(Sampler):
    ...
    def create_samples_dict(self, paths):
        ...
        if self.algo.safety_constraint:
            ...
            samples_data['safety_rescale'] = len(samples_data['safety_values']) / sum(
                [len(paths) for paths in self.experience_replay])
    ...
    def record_statistics(self, itr, paths, evs, samples_data):
        ...
        if self.algo.safety_constraint:
            ...
            if self.use_safety_bonus:
                ...
                if self.algo.batch_aggregate_n > 1:
                    new_safety_robust_returns = np.array(
                        [np.sum(path["safety_robust_rewards"]) for path in self.experience_replay[-1]])
